...
На рабочей машине был установлен VirtualBox версии 5.2 и Vagrant версии 2.0.2, создана конфигурация для развертывания виртуальных машин подсистемы 
в локальном окружении. Настроен Provisioning приложения при помощи Ansible, доработаны роли **db** и **app** для их корректной работы в 
локальном окружении.
...
Проверена корректность работы экземпляра приложения, развернутого с помощью Vagrant.


##### Задача 2. Тестирование ролей Ansible.

Тестирование проводилось внутри среды `virtualenv` версии 15.1.0. Директория окружения, созданная virtualenv, добавлена в .gitignore.
Стоит отметить, что для корректной работы Molecule в окружении virtualenv (при создании инстанса ВМ требуется `libselinux-python`) пришлось отключить 
selinux на рабчоей машине, т.к. иначе используется библиотека `libselinux-python`, которой нет в виртуальном окружении, кроме того, явное указание 
интерпретатора python или ключ `--system-site-packages` при инициализации virtualenv результата не дали.
Тестирование производилось с помощью Molecule версии 2.11.0, версия Ansible: 2.5.0.
Для роли **db** инициализирован тестовый сценарий для **Molecule**. Создан тестовый экземпляр виртуальной машины, применена конфигурация роли, 
запущены тесты с использованием **Testinfra**. 
К существующим тестам был добавлен тест порта MongoDB (с помощью модуля `Socket`).

Плейбуки `packer_app.yml` и `packer_db.yml`, используемые Packer для провижининга, переработаны на использование ролей вместо тасков.
Для корректного взаимодействия Packer и Ansible в шаблоны Packer в блок описания Ansible Provisioner было задано свойство `ansible_env_vars`,
при этом указан относительный путь к директории ролей Ansible.
Предполагается, что команда `packer build` запускается из директории с шаблонами.

Настроено оповещение о тестовом билде репозитория с использованием Travis CI в канал #dmitry-bredikhin Slack-чата "DevOps team".
